/* Generated automatically by the program `genconstants'
   from the machine description file `md'.  */

#ifndef GCC_INSN_CONSTANTS_H
#define GCC_INSN_CONSTANTS_H

#define CMP_CMP 0
#define DOM_CC_NX_OR_Y 1
#define DOM_CC_X_OR_Y 2
#define NUM_OF_COND_CMP 4
#define CC_REGNUM 100
#define WCGR3 99
#define SP_REGNUM 13
#define R1_REGNUM 1
#define PC_REGNUM 15
#define WCGR0 96
#define VFPCC_REGNUM 101
#define CMP_CMN 2
#define WCGR2 98
#define R0_REGNUM 0
#define WCGR1 97
#define CMN_CMP 1
#define LR_REGNUM 14
#define DOM_CC_X_AND_Y 0
#define CMN_CMN 3
#define IP_REGNUM 12
#define LAST_ARM_REGNUM 15

enum unspec {
  UNSPEC_PUSH_MULT = 0,
  UNSPEC_PIC_SYM = 1,
  UNSPEC_PIC_BASE = 2,
  UNSPEC_PRLG_STK = 3,
  UNSPEC_REGISTER_USE = 4,
  UNSPEC_CHECK_ARCH = 5,
  UNSPEC_WSHUFH = 6,
  UNSPEC_WACC = 7,
  UNSPEC_TMOVMSK = 8,
  UNSPEC_WSAD = 9,
  UNSPEC_WSADZ = 10,
  UNSPEC_WMACS = 11,
  UNSPEC_WMACU = 12,
  UNSPEC_WMACSZ = 13,
  UNSPEC_WMACUZ = 14,
  UNSPEC_CLRDI = 15,
  UNSPEC_WALIGNI = 16,
  UNSPEC_TLS = 17,
  UNSPEC_PIC_LABEL = 18,
  UNSPEC_PIC_OFFSET = 19,
  UNSPEC_GOTSYM_OFF = 20,
  UNSPEC_THUMB1_CASESI = 21,
  UNSPEC_RBIT = 22,
  UNSPEC_SYMBOL_OFFSET = 23,
  UNSPEC_MEMORY_BARRIER = 24,
  UNSPEC_UNALIGNED_LOAD = 25,
  UNSPEC_UNALIGNED_STORE = 26,
  UNSPEC_PIC_UNIFIED = 27,
  UNSPEC_LL = 28,
  UNSPEC_VRINTZ = 29,
  UNSPEC_VRINTP = 30,
  UNSPEC_VRINTM = 31,
  UNSPEC_VRINTR = 32,
  UNSPEC_VRINTX = 33,
  UNSPEC_VRINTA = 34,
  UNSPEC_GOT_PREL_SYM = 35,
  UNSPEC_WADDC = 36,
  UNSPEC_WABS = 37,
  UNSPEC_WQMULWMR = 38,
  UNSPEC_WQMULMR = 39,
  UNSPEC_WQMULWM = 40,
  UNSPEC_WQMULM = 41,
  UNSPEC_WQMIAxyn = 42,
  UNSPEC_WQMIAxy = 43,
  UNSPEC_TANDC = 44,
  UNSPEC_TORC = 45,
  UNSPEC_TORVSC = 46,
  UNSPEC_TEXTRC = 47,
  UNSPEC_ASHIFT_SIGNED = 48,
  UNSPEC_ASHIFT_UNSIGNED = 49,
  UNSPEC_CRC32B = 50,
  UNSPEC_CRC32H = 51,
  UNSPEC_CRC32W = 52,
  UNSPEC_CRC32CB = 53,
  UNSPEC_CRC32CH = 54,
  UNSPEC_CRC32CW = 55,
  UNSPEC_AESD = 56,
  UNSPEC_AESE = 57,
  UNSPEC_AESIMC = 58,
  UNSPEC_AESMC = 59,
  UNSPEC_SHA1C = 60,
  UNSPEC_SHA1M = 61,
  UNSPEC_SHA1P = 62,
  UNSPEC_SHA1H = 63,
  UNSPEC_SHA1SU0 = 64,
  UNSPEC_SHA1SU1 = 65,
  UNSPEC_SHA256H = 66,
  UNSPEC_SHA256H2 = 67,
  UNSPEC_SHA256SU0 = 68,
  UNSPEC_SHA256SU1 = 69,
  UNSPEC_VMULLP64 = 70,
  UNSPEC_LOAD_COUNT = 71,
  UNSPEC_VABD = 72,
  UNSPEC_VABDL = 73,
  UNSPEC_VADD = 74,
  UNSPEC_VADDHN = 75,
  UNSPEC_VADDL = 76,
  UNSPEC_VADDW = 77,
  UNSPEC_VBSL = 78,
  UNSPEC_VCAGE = 79,
  UNSPEC_VCAGT = 80,
  UNSPEC_VCEQ = 81,
  UNSPEC_VCGE = 82,
  UNSPEC_VCGEU = 83,
  UNSPEC_VCGT = 84,
  UNSPEC_VCGTU = 85,
  UNSPEC_VCLS = 86,
  UNSPEC_VCONCAT = 87,
  UNSPEC_VCVT = 88,
  UNSPEC_VCVT_N = 89,
  UNSPEC_VEXT = 90,
  UNSPEC_VHADD = 91,
  UNSPEC_VHSUB = 92,
  UNSPEC_VLD1 = 93,
  UNSPEC_VLD1_LANE = 94,
  UNSPEC_VLD2 = 95,
  UNSPEC_VLD2_DUP = 96,
  UNSPEC_VLD2_LANE = 97,
  UNSPEC_VLD3 = 98,
  UNSPEC_VLD3A = 99,
  UNSPEC_VLD3B = 100,
  UNSPEC_VLD3_DUP = 101,
  UNSPEC_VLD3_LANE = 102,
  UNSPEC_VLD4 = 103,
  UNSPEC_VLD4A = 104,
  UNSPEC_VLD4B = 105,
  UNSPEC_VLD4_DUP = 106,
  UNSPEC_VLD4_LANE = 107,
  UNSPEC_VMAX = 108,
  UNSPEC_VMIN = 109,
  UNSPEC_VMLA = 110,
  UNSPEC_VMLAL = 111,
  UNSPEC_VMLA_LANE = 112,
  UNSPEC_VMLAL_LANE = 113,
  UNSPEC_VMLS = 114,
  UNSPEC_VMLSL = 115,
  UNSPEC_VMLS_LANE = 116,
  UNSPEC_VMLSL_LANE = 117,
  UNSPEC_VMOVL = 118,
  UNSPEC_VMOVN = 119,
  UNSPEC_VMUL = 120,
  UNSPEC_VMULL = 121,
  UNSPEC_VMUL_LANE = 122,
  UNSPEC_VMULL_LANE = 123,
  UNSPEC_VPADAL = 124,
  UNSPEC_VPADD = 125,
  UNSPEC_VPADDL = 126,
  UNSPEC_VPMAX = 127,
  UNSPEC_VPMIN = 128,
  UNSPEC_VPSMAX = 129,
  UNSPEC_VPSMIN = 130,
  UNSPEC_VPUMAX = 131,
  UNSPEC_VPUMIN = 132,
  UNSPEC_VQABS = 133,
  UNSPEC_VQADD = 134,
  UNSPEC_VQDMLAL = 135,
  UNSPEC_VQDMLAL_LANE = 136,
  UNSPEC_VQDMLSL = 137,
  UNSPEC_VQDMLSL_LANE = 138,
  UNSPEC_VQDMULH = 139,
  UNSPEC_VQDMULH_LANE = 140,
  UNSPEC_VQDMULL = 141,
  UNSPEC_VQDMULL_LANE = 142,
  UNSPEC_VQMOVN = 143,
  UNSPEC_VQMOVUN = 144,
  UNSPEC_VQNEG = 145,
  UNSPEC_VQSHL = 146,
  UNSPEC_VQSHL_N = 147,
  UNSPEC_VQSHLU_N = 148,
  UNSPEC_VQSHRN_N = 149,
  UNSPEC_VQSHRUN_N = 150,
  UNSPEC_VQSUB = 151,
  UNSPEC_VRECPE = 152,
  UNSPEC_VRECPS = 153,
  UNSPEC_VREV16 = 154,
  UNSPEC_VREV32 = 155,
  UNSPEC_VREV64 = 156,
  UNSPEC_VRSQRTE = 157,
  UNSPEC_VRSQRTS = 158,
  UNSPEC_VSHL = 159,
  UNSPEC_VSHLL_N = 160,
  UNSPEC_VSHL_N = 161,
  UNSPEC_VSHR_N = 162,
  UNSPEC_VSHRN_N = 163,
  UNSPEC_VSLI = 164,
  UNSPEC_VSRA_N = 165,
  UNSPEC_VSRI = 166,
  UNSPEC_VST1 = 167,
  UNSPEC_VST1_LANE = 168,
  UNSPEC_VST2 = 169,
  UNSPEC_VST2_LANE = 170,
  UNSPEC_VST3 = 171,
  UNSPEC_VST3A = 172,
  UNSPEC_VST3B = 173,
  UNSPEC_VST3_LANE = 174,
  UNSPEC_VST4 = 175,
  UNSPEC_VST4A = 176,
  UNSPEC_VST4B = 177,
  UNSPEC_VST4_LANE = 178,
  UNSPEC_VSTRUCTDUMMY = 179,
  UNSPEC_VSUB = 180,
  UNSPEC_VSUBHN = 181,
  UNSPEC_VSUBL = 182,
  UNSPEC_VSUBW = 183,
  UNSPEC_VTBL = 184,
  UNSPEC_VTBX = 185,
  UNSPEC_VTRN1 = 186,
  UNSPEC_VTRN2 = 187,
  UNSPEC_VTST = 188,
  UNSPEC_VUZP1 = 189,
  UNSPEC_VUZP2 = 190,
  UNSPEC_VZIP1 = 191,
  UNSPEC_VZIP2 = 192,
  UNSPEC_MISALIGNED_ACCESS = 193,
  UNSPEC_VCLE = 194,
  UNSPEC_VCLT = 195,
  UNSPEC_NVRINTZ = 196,
  UNSPEC_NVRINTP = 197,
  UNSPEC_NVRINTM = 198,
  UNSPEC_NVRINTX = 199,
  UNSPEC_NVRINTA = 200,
  UNSPEC_NVRINTN = 201
};
#define NUM_UNSPEC_VALUES 202
extern const char *const unspec_strings[];

enum unspecv {
  VUNSPEC_BLOCKAGE = 0,
  VUNSPEC_EPILOGUE = 1,
  VUNSPEC_THUMB1_INTERWORK = 2,
  VUNSPEC_ALIGN = 3,
  VUNSPEC_POOL_END = 4,
  VUNSPEC_POOL_1 = 5,
  VUNSPEC_POOL_2 = 6,
  VUNSPEC_POOL_4 = 7,
  VUNSPEC_POOL_8 = 8,
  VUNSPEC_POOL_16 = 9,
  VUNSPEC_TMRC = 10,
  VUNSPEC_TMCR = 11,
  VUNSPEC_ALIGN8 = 12,
  VUNSPEC_WCMP_EQ = 13,
  VUNSPEC_WCMP_GTU = 14,
  VUNSPEC_WCMP_GT = 15,
  VUNSPEC_EH_RETURN = 16,
  VUNSPEC_ATOMIC_CAS = 17,
  VUNSPEC_ATOMIC_XCHG = 18,
  VUNSPEC_ATOMIC_OP = 19,
  VUNSPEC_LL = 20,
  VUNSPEC_LDRD_ATOMIC = 21,
  VUNSPEC_SC = 22,
  VUNSPEC_LAX = 23,
  VUNSPEC_SLX = 24,
  VUNSPEC_LDA = 25,
  VUNSPEC_STL = 26
};
#define NUM_UNSPECV_VALUES 27
extern const char *const unspecv_strings[];

#endif /* GCC_INSN_CONSTANTS_H */
